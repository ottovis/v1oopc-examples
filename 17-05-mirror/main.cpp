...
public:
   window_mirror( window & w ): 
      window( hwlib::xy( w.size.x / 2, w.size.y ) ),
      w( w )
   {}
   
   void write_implementation( hwlib::xy pixel, hwlib::color c ) override {
      if( pixel.x <= ( w.size.x / 2 )){
         w.write( pixel, c );
         w.write( hwlib::xy( ( w.size.x - 1 ) - pixel.x, pixel.y ), c );
      }   
   }
...
int main(){
   ...
   line right( w, hwlib::xy( 63,  0 ), hwlib::xy( 63, 63 ) );
   line bottom( w, hwlib::xy(   0, 63 ), hwlib::xy( 63, 63 ) );
